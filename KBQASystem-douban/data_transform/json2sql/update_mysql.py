"""
结合pre_process.py使用，如果已经使用hadoop进行ETL，同样不需要执行此代码
"""
...
def update_movie_info():
    """
    爬取的电影名都是类似:"放牛班的春天 Les choristes"这种中英文混合的结构，进行修改，保留中文部分，如果只有英文名就只保留英文部分
    :return:
    """
    movie_info_file_path = '../../data/new/movie_info.txt'
    ...
    new_movie_name_file_path = '../../data/new/new_movie_name.txt'
    ...
    new_movie_info_file_path = '../../data/new/movie_info.txt'
...
def update_movie_person_info():
    ...
    movie_person_name_file_path = '../../data/new/movie_person_name.txt'
    with open(movie_person_name_file_path, 'w') as f:
        for name in name_list:
            f.write(json.dumps(name, ensure_ascii=False) + '\n')

    movie_person_file_path = '../../data/new/movie_person_info.txt'
    ...
    for person in movie_person_json_list:
        ...
        if person_id in movie_json_dict:
...
def update_book_person_info():
    # 先提取正常姓名， 并写入到文件之中
    book_person_file_path = '../../data/new/book_person_info.txt'
    ...
    new_book_person_name_file_path = '../../data/new/new_book_person_name.txt'
...
def update_sql_movie_info():
    ...
    """
    # 读取文件
    movie_file_path = '../../data/new/movie_info.txt'
    ...
    try:
        with kgqa_connection.cursor() as cursor:
            for movie in movie_json_list:
                ...
                movie_info_update_sql = "UPDATE `movie_info` SET `movie_info_name` =" + '\"' + str(
                    movie_info_name) + '\"' + "WHERE `movie_info_id`=" + str(movie_info_id)
